...
import { ProjNameUnique, Store } from '../../services/Project';
...
const Add = () => {
  ...
  const handleFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < e.target.files.length; i++) {
      ...
      if (files.length + newFiles.length >= 3) break; // max 3 files
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='px-3'>
          ...
          <div className="mt-3">
            ...
            <input
              ...
              {...register("title", {
                ...
                validate: async (value) => await ProjNameUnique(value, user?.id),
              })}
            />
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mt-3'>
          ...
          <div className="mt-3">
            ...
            />
            {errors.category && (<span className="text-red-500 mt-1 text-sm"> {errors.category.message}</span>)}
          </div>
          ...
          <div className="mt-3">
            ...
            {errors.skills && (<span className="text-red-500 mt-1 text-sm"> {errors.skills.message}</span>)}
          </div>
            ...
            {files.map((f, i) => (
              ...
              >
                ...
                {f.preview ? (
                  ...
                  <p className="text-sm text-gray-700 dark:text-gray-100 text-center">{f.file.name}</p>
                )}
              </div>
            ))}

            {/* + Add new box (hidden if 3 uploads) */}
            {files.length < 3 && (
              <label className="border border-dashed rounded w-32 h-32 flex items-center justify-center cursor-pointer hover:border-gray-400 relative">
                <input
                  type="file"
                  className="hidden"
